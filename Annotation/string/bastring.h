...
private:
  struct Rep {
    ...
    */
    size_t len, res, ref;
    bool selfish;
    //reinterpret_cast 无关类型之间的转换
    charT* data () { return reinterpret_cast<charT *>(this + 1); }
    charT& operator[] (size_t s) { return data () [s]; }
    //复制  或者 引用数ref加一,返回 data 
    charT* grab () { if (selfish) return clone (); ++ref; return data (); } 
    //释放 , 如果 ref 不为零，引用数-1(delete 被重载了，实现在bastring.cc中)
...
public:
// 类型:
  typedef	   traits		traits_type; //第二参数
  typedef typename traits::char_type	value_type; //第二参数的类型
  ...
  static const size_type npos = static_cast<size_type>(-1);//特殊值，表示未找到或所有剩余的字符
...
// _lib.string.cons_ construct/copy/destroy: 构造、复制、销毁函数

  //effective 中提及的 operator应该返回 *this ,并处理 "自我复制"
  basic_string& operator= (const basic_string& str)
    {
      /*
        effective 避免自我复制,先释放内存，在“复制”
      */
...
private:
  ...
  static Rep nilRep; //静态成员，所有实例共享一个
  charT *dat; //数据
};
